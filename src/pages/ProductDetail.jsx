...
export default function ProductDetail() {
  ...
  useEffect(() => {
    Promise.all([getProduct(id), getReviews(id)])
      .then(([pRes, rRes]) => { setProduct(pRes.data.product); setReviews(rRes.data.reviews); })
  ...
  return (
    <main data-testid="product-detail-page" style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, marginBottom: 60 }}>
        {/* Image */}
        <div data-testid="product-image-container" style={{ background: 'var(--bg-card-2)', borderRadius: 'var(--r-xl)', padding: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 400 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-3)', marginBottom: 16 }}>by <strong>{product.brand}</strong> · {product.category}</div>
      ...
      <section data-testid="reviews-section">
        ...
        {reviews.length === 0 ? <p style={{ color: 'var(--text-3)' }}>No reviews yet. Be the first!</p> : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 40 }}>
            {reviews.map(r => (
              <div key={r._id} data-testid="review-card" style={{ background: 'var(--bg-card)', borderRadius: 'var(--r-lg)', padding: 20, border: '1px solid var(--border)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text-1)' }}>{r.user?.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--star)' }}>{'⭐'.repeat(r.rating)}</div>
                  </div>
                </div>
        ...
        {isLoggedIn && (
          <form data-testid="review-form" onSubmit={handleReview} style={{ background: 'var(--bg-card)', borderRadius: 'var(--r-lg)', padding: 24, border: '1px solid var(--border)', maxWidth: 600 }}>
            <h3 style={{ marginBottom: 20, color: 'var(--text-1)' }}>Write a Review</h3>
            <div style={{ marginBottom: 16 }}>
              <label style={{ fontSize: 13, color: 'var(--text-2)', display: 'block', marginBottom: 6 }}>Rating</label>
              <select data-testid="review-rating" value={reviewForm.rating} onChange={e => setReviewForm(f => ({ ...f, rating: Number(e.target.value) }))}
                style={{ padding: '8px 12px', borderRadius: 'var(--r-sm)', border: '1.5px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-1)' }}>
            ...
            <input placeholder="Review title (optional)" value={reviewForm.title} onChange={e => setReviewForm(f => ({ ...f, title: e.target.value }))}
              style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--r-sm)', border: '1.5px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-1)', marginBottom: 12 }} />
            <textarea data-testid="review-comment" required placeholder="Share your experience..." value={reviewForm.comment} onChange={e => setReviewForm(f => ({ ...f, comment: e.target.value }))}
              rows={4} style={{ width: '100%', padding: '10px 14px', borderRadius: 'var(--r-sm)', border: '1.5px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-1)', resize: 'vertical', marginBottom: 16 }} />
            <button data-testid="submit-review-btn" type="submit"
