      ...
      <section data-testid="hero-section" style={{ background: 'linear-gradient(135deg, #0d0f10 0%, #1a1c1e 50%, #0d0f10 100%)', padding: '80px 20px', textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 18, color: '#9ca3af', marginBottom: 36, lineHeight: 1.7 }}>
            Discover thousands of products across Electronics, Books, Clothing, and more. Shop smarter with Scelta.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={() => navigate('/products?featured=true')}
              style={{ padding: '14px 32px', background: 'transparent', color: '#fff', border: '1.5px solid rgba(255,255,255,0.2)', borderRadius: 'var(--r-full)', fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>
              View Deals
            </button>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 16 }}>
            {categories.map(cat => (
              <Link key={cat.id} to={`/products?category=${cat.id}`} data-testid={`category-card-${cat.id}`}
                style={{ background: 'var(--bg-card)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', padding: '24px 16px', textAlign: 'center', transition: 'all 0.2s' }}
