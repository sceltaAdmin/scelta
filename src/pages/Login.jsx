      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--r-xl)', border: '1px solid var(--border)', padding: '40px', width: '100%', maxWidth: 440, boxShadow: 'var(--shadow-lg)' }}>
        ...
        <form data-testid="login-form" onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            ...
            <input data-testid="login-email" type="email" required value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <input data-testid="login-password" type="password" required value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))}
              placeholder="••••••••"
              style={{ width: '100%', padding: '12px 16px', borderRadius: 'var(--r-md)', border: '1.5px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-1)', fontSize: 14 }} />
          </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: 'var(--text-3)' }}>
          Don't have an account? <Link to="/register" data-testid="go-to-register" style={{ color: 'var(--fire)', fontWeight: 600 }}>Register</Link>
        </p>
      </div>
    </main>
  );
