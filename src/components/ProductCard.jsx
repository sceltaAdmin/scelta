...
export default function ProductCard({ product }) {
  ...
  return (
    <Link to={`/products/${product._id}`} data-testid={`product-card-${product._id}`}
      style={{ background: 'var(--bg-card)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', overflow: 'hidden', display: 'block', transition: 'all 0.2s', textDecoration: 'none' }}
      ...
      <div style={{ position: 'relative', background: 'var(--bg-card-2)', aspectRatio: '1', overflow: 'hidden' }}>
        <img src={product.image} alt={product.name} loading="lazy"
          style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 16, transition: 'transform 0.3s' }} />
        {product.badge && (
          <span data-testid="product-badge" style={{ position: 'absolute', top: 10, left: 10, background: 'var(--fire)', color: '#fff', fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 'var(--r-full)' }}>{product.badge}</span>
        )}
        <button data-testid={`wishlist-btn-${product._id}`} onClick={handleWishlist}
          style={{ position: 'absolute', top: 10, right: 10, background: inWish ? 'var(--fire-pale)' : 'rgba(255,255,255,0.9)', border: 'none', borderRadius: '50%', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, cursor: 'pointer' }}>
          {inWish ? '❤️' : '🤍'}
        </button>
      </div>
      ...
      <div style={{ padding: '14px 16px' }}>
        <div style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 4, textTransform: 'capitalize' }}>{product.brand}</div>
        ...
        <button data-testid={`add-to-cart-${product._id}`} onClick={handleAddToCart}
          style={{ width: '100%', padding: '10px', background: 'var(--fire)', color: '#fff', border: 'none', borderRadius: 'var(--r-full)', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'background 0.2s' }}>
          Add to Cart
        </button>
      </div>
    </Link>
