      ...
      <nav style={{ background: 'var(--bg-nav)', padding: '0 20px', position: 'sticky', top: 0, zIndex: 1000 }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 16, height: 64 }}>
          ...
          <div ref={searchRef} style={{ flex: 1, maxWidth: 600, position: 'relative' }}>
            <form onSubmit={handleSearchSubmit} style={{ display: 'flex', borderRadius: 'var(--r-full)', overflow: 'hidden', background: '#fff' }}>
              <input
                ...
                style={{ flex: 1, border: 'none', padding: '10px 16px', fontSize: 14, outline: 'none', background: 'transparent' }}
              />
              <button data-testid="search-btn" type="submit" style={{ padding: '10px 20px', background: 'var(--fire)', border: 'none', color: '#fff', fontSize: 16 }}>🔍</button>
            </form>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            ...
            {/* Wishlist */}
            <Link to="/wishlist" data-testid="nav-wishlist" style={{ position: 'relative', background: 'transparent', border: 'none', color: '#d1d5db', padding: '8px 10px', borderRadius: 'var(--r-md)', fontSize: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              🤍
              {wishlist.length > 0 && (
                <span style={{ position: 'absolute', top: 2, right: 2, background: 'var(--fire)', color: '#fff', fontSize: 10, fontWeight: 700, borderRadius: 'var(--r-full)', minWidth: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>{wishlist.length}</span>
              )}
            </Link>

            {/* Cart */}
            <Link to="/cart" data-testid="nav-cart" style={{ position: 'relative', background: 'transparent', border: 'none', color: '#d1d5db', padding: '8px 10px', borderRadius: 'var(--r-md)', fontSize: 20, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              🛒
              {cartCount > 0 && (
                <span data-testid="cart-badge" style={{ position: 'absolute', top: 2, right: 2, background: 'var(--fire)', color: '#fff', fontSize: 10, fontWeight: 700, borderRadius: 'var(--r-full)', minWidth: 16, height: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>{cartCount}</span>
              )}
            </Link>
      ...
      <div style={{ background: 'var(--bg-nav)', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '0 20px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', gap: 4, overflowX: 'auto' }}>
          {[['All', ''], ['Electronics', 'electronics'], ['Books', 'books'], ['Clothing', 'clothing'], ['Home & Kitchen', 'home-kitchen'], ['Sports', 'sports'], ['Toys', 'toys']].map(([label, cat]) => (
            <Link key={cat} to={cat ? `/products?category=${cat}` : '/products'}
              data-testid={`cat-${cat || 'all'}`}
              style={{ padding: '10px 14px', fontSize: 13, color: '#9ca3af', whiteSpace: 'nowrap', borderBottom: '2px solid transparent', transition: 'all 0.15s' }}
              onMouseEnter={e => { e.target.style.color = '#fff'; e.target.style.borderBottomColor = 'var(--fire)'; }}
              onMouseLeave={e => { e.target.style.color = '#9ca3af'; e.target.style.borderBottomColor = 'transparent'; }}>
              {label}
            </Link>
          ))}
        </div>
