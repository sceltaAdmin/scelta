...
export default function Checkout() {
  ...
  const handlePlaceOrder = async () => {
    ...
    // Browser alert confirmation — for Selenium testing
        ...
        {['Shipping', 'Delivery Slot', 'Payment'].map((s, i) => (
          <div key={s} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: step > i ? 'var(--fire)' : step === i + 1 ? 'var(--fire)' : 'var(--border)', color: step >= i + 1 ? '#fff' : 'var(--text-3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14, marginBottom: 6 }}>
                {step > i + 1 ? '✓' : i + 1}
              </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 10 }}>
                  {TIME_SLOTS.map(slot => (
                    <button key={slot}
                      ...
                      style={{ padding: '12px 16px', borderRadius: 'var(--r-md)', border: '1.5px solid', borderColor: timeSlot === slot ? 'var(--fire)' : 'var(--border)', background: timeSlot === slot ? 'var(--fire-pale)' : 'var(--bg-card)', color: timeSlot === slot ? 'var(--fire)' : 'var(--text-1)', fontWeight: timeSlot === slot ? 700 : 400, cursor: 'pointer', fontSize: 13, transition: 'all 0.2s' }}>
                      {slot}
                    </button>
                  ))}
                </div>
          ...
          {step === 3 && (
            <div data-testid="payment-form" style={{ background: 'var(--bg-card)', borderRadius: 'var(--r-lg)', border: '1px solid var(--border)', padding: 28 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
                {[
                  { id: 'COD', label: 'Cash on Delivery', icon: '💵', desc: 'Pa
cat > ~/Desktop/scelta/frontend/src/pages/Checkout.jsx << 'ENDOFFILE'
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useCart } from '../context/CartContext';
import { useAuth } from '../context/AuthContext';
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
                {[
                  ...
                  <label key={method.id} data-testid={`payment-${method.id.toLowerCase()}`}
                    style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 20px', borderRadius: 'var(--r-lg)', border: `1.5px solid ${paymentMethod === method.id ? 'var(--fire)' : 'var(--border)'}`, background: paymentMethod === method.id ? 'var(--fire-pale)' : 'var(--bg-card)', cursor: 'pointer', transition: 'all 0.2s' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
            {items.map(item => (
              <div key={item._id} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-1)', flexShrink: 0 }}>₹{(item.price * item.quantity).toLocaleString()}</div>
              </div>
            ))}
          </div>
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'var(--text-2)', marginBottom: 6 }}><span>Subtotal</span><span>₹{subtotal.toLocaleString()}</span></div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: delivery === 0 ? 'var(--green)' : 'var(--text-2)', marginBottom: 12 }}><span>Delivery</span><span>{delivery === 0 ? 'FREE' : `₹${delivery}`}</span></div>
            {deliveryDate && <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 8 }}>📅 {deliveryDate}</div>}
            {timeSlot && <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 12 }}>🕐 {timeSlot}</div>}
            <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: 18, color: 'var(--text-1)' }}><span>Total</span><span>₹{total.toLocaleString()}</span></div>
          </div>
        </div>
      </div>
    </main>
