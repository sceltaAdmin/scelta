...
export function CartProvider({ children }) {
  ...
  const fetchCart = async () => {
    try { const res = await getCart(); setCart(res.data.cart); } catch {}
  };
  ...
  const updateItem = async (itemId, quantity) => {
    try { const res = await updateCartItem(itemId, { quantity }); setCart(res.data.cart); } catch {}
  };

  const removeItem = async (itemId) => {
    try { await removeCartItem(itemId); await fetchCart(); toast.success('Removed from cart'); } catch {}
  };
