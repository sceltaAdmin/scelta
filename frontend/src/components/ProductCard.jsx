    ...
    <Link to={`/products/${product._id}`} data-testid={`product-card-${product._id}`}
      ...
      <div style={{ position: 'relative', background: 'var(--bg-card-2)', aspectRatio: '1', overflow: 'hidden' }}>
        ...
        <button data-testid={`wishlist-btn-${product._id}`} onClick={handleWishlist}
          style={{ position: 'absolute', top: 10, right: 10, background: inWish ? 'var(--fire-pale)' : 'rgba(255,255,255,0.9)', border: 'none', borderRadius: '50%', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>
          {inWish ? '❤️' : '🤍'}
        </button>
      </div>
