...
export default function Navbar() {
  ...
  const SearchSuggestions = ({ onSelect }) => (
    showSugg && suggestions.length > 0 ? (
      <div data-testid="search-suggestions"
        ...
        {suggestions.map(p => (
          <div key={p._id}
            onClick={() => { navigate(`/products/${p._id}`); setShowSugg(false); setSearch(''); onSelect?.(); }}
  ...
  return (
    <header data-testid="navbar" style={{ position: 'sticky', top: 0, zIndex: 1000 }}>
            ...
            <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 16, height: 64 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto', flexShrink: 0 }}>
                ...
                <button data-testid="theme-toggle" onClick={() => setTheme(t => t === 'light' ? 'dark' : 'light')}
                  style={{ background: 'transparent', border: 'none', color: '#9ca3af', padding: '8px', borderRadius: 8, fontSize: 17, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                ...
                <Link to="/wishlist" data-testid="nav-wishlist"
                  style={{ position: 'relative', padding: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af', fontSize: 17, borderRadius: 8 }}
                  onMouseEnter={e => e.currentTarget.style.color = '#fff'}
                  onMouseLeave={e => e.currentTarget.style.color = '#9ca3af'}>
                  {wishlist.length > 0 ? '❤️' : '🤍'}
                  {wishlist.length > 0 && (
                ...
                {isLoggedIn ? (
                  ...
                  <Link to="/login" data-testid="nav-login-btn"
                    style={{ padding: '8px 18px', background: 'var(--fire)', color: '#fff', borderRadius: 999, fontSize: 13, fontWeight: 600, marginLeft: 4 }}
              ...
              {CATEGORIES.map(({ id, label }) => {
                ...
                return (
                  <Link key={id}
                    ...
                    style={{ padding: '10px 14px', fontSize: 12.5, whiteSpace: 'nowrap', display: 'block', color: isActive ? '#fff' : '#9ca3af', borderBottom: isActive ? '2px solid var(--fire)' : '2px solid transparent', fontWeight: isActive ? 600 : 400, transition: 'all 0.15s', flexShrink: 0 }}
      ...
      {isMobile && (
        <>
          <nav style={{ background: 'var(--bg-nav)', padding: '0 12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: 52 }}>

              {/* Logo */}
              <Link to="/" data-testid="nav-logo" style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }}>
                <img src="/logo.png" alt="Scelta" style={{ height: 26, width: 26, objectFit: 'contain' }} />
                <span style={{ fontFamily: 'var(--font-display)', fontSize: 18, color: '#fff', marginLeft: -4 }}>
                  celt<span style={{ color: 'var(--fire)' }}>a</span>
                </span>
              </Link>
              ...
              <button onClick={() => setShowMobileSearch(s => !s)}
                style={{ background: 'transparent', border: 'none', color: '#9ca3af', padding: '6px', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                🔍
              </button>

              {/* Theme */}
              <button data-testid="theme-toggle" onClick={() => setTheme(t => t === 'light' ? 'dark' : 'light')}
                style={{ background: 'transparent', border: 'none', color: '#9ca3af', padding: '6px', fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                {theme === 'light' ? '🌙' : '☀️'}
              </button>

              {/* Cart */}
              <Link to="/cart" data-testid="nav-cart"
                style={{ position: 'relative', padding: '6px', display: 'flex', alignItems: 'center', color: '#9ca3af', fontSize: 18 }}>
                🛒
                {cartCount > 0 && (
                  <span data-testid="cart-badge"
                    style={{ position: 'absolute', top: 0, right: 0, background: 'var(--fire)', color: '#fff', fontSize: 8, fontWeight: 700, borderRadius: 999, minWidth: 14, height: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 3px' }}>
                    {cartCount}
                  </span>
                )}
              </Link>
              ...
              {isLoggedIn ? (
                <div ref={userRef} style={{ position: 'relative' }}>
                  <button data-testid="user-menu-btn" onClick={() => setShowUserMenu(v => !v)}
                    style={{ display: 'flex', alignItems: 'center', gap: 4, background: '#1a1c1e', border: '1px solid #2a2d30', borderRadius: 999, padding: '4px 8px 4px 4px', cursor: 'pointer' }}>
                    <div style={{ width: 26, height: 26, borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg, var(--fire), var(--fire-dark))', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {user?.avatar
                        ? <img src={user.avatar} alt={user.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={e => { e.target.style.display = 'none'; }} />
                  ...
                  {showUserMenu && (
                    <div data-testid="user-dropdown"
                      style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--r-lg)', boxShadow: 'var(--shadow-xl)', minWidth: 180, zIndex: 9999, overflow: 'hidden' }}>
                      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)', background: 'var(--bg-card-2)' }}>
                        <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--text-1)' }}>{user.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-3)' }}>{user.email}</div>
                      </div>
                      {[['👤', 'My Profile', '/profile'], ['📦', 'My Orders', '/orders'], ['🤍', 'Wishlist', '/wishlist'], ['🛒', 'Cart', '/cart']].map(([icon, label, path]) => (
                        <Link key={path} to={path} onClick={() => setShowUserMenu(false)}
                          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', fontSize: 13, color: 'var(--text-1)', borderBottom: '1px solid var(--border)' }}
                          onMouseEnter={e => e.currentTarget.style.background = 'var(--snow)'}
                          onMouseLeave={e => e.currentTarget.style.background = ''}>
                          <span>{icon}</span>{label}
                        </Link>
                      ...
                      <button data-testid="logout-btn" onClick={handleLogout}
                        style={{ width: '100%', padding: '10px 14px', fontSize: 13, color: 'var(--red)', background: 'none', border: 'none', textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span>🚪</span> Logout
                      </button>
                    </div>
                  )}
                ...
                <Link to="/login" data-testid="nav-login-btn"
                  style={{ padding: '6px 14px', background: 'var(--fire)', color: '#fff', borderRadius: 999, fontSize: 12, fontWeight: 600 }}>
                  Login
                </Link>
              )}
            </div>
            ...
            {showMobileSearch && (
              <div ref={searchRef} style={{ paddingBottom: 10 }}>
                <form onSubmit={handleSearchSubmit}
                ...
                {showSugg && suggestions.length > 0 && (
                  <div data-testid="search-suggestions"
                    ...
                    {suggestions.map(p => (
                      <div key={p._id}
                        onClick={() => { navigate(`/products/${p._id}`); setShowSugg(false); setSearch(''); setShowMobileSearch(false); }}
          ...
          {/* Mobile category nav */}
          <div style={{ background: 'var(--bg-nav)', borderTop: '1px solid rgba(255,255,255,0.05)', overflowX: 'auto', scrollbarWidth: 'none' }}>
              ...
              {CATEGORIES.map(({ id, label }) => {
                ...
                return (
                  <Link key={id}
                    to={id === 'coupons' ? '/coupons' : id ? `/products?category=${id}` : '/products'}
                    data-testid={`cat-${id || 'all'}`}
                    style={{ padding: '8px 10px', fontSize: 11, whiteSpace: 'nowrap', display: 'block', color: isActive ? '#fff' : '#9ca3af', borderBottom: isActive ? '2px solid var(--fire)' : '2px solid transparent', fontWeight: isActive ? 600 : 400, flexShrink: 0 }}>
                    {label}
                  </Link>
                );
              })}
